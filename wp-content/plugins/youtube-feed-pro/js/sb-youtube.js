                ...
                if (ctaMatches !== null) {
                    ...
                    if (urlMatches !== null) {
                        var url = urlMatches[0].trim(),
                            sby_views = ctaMatches[0].replace('{Link:','').replace('}','').replace(url,'').replace('  ',' ').trim();
                        cta = {
                            callback: 'link',
                            url: url,
                            text: ''
        ...
        function SbyFeedPro(el, index, settings) {
            ...
            this.afterItemRevealed = function($item) {
                ...
                if ($item.find('.sby_caption').length && ! $item.find('.sby_caption').hasClass('sby_full_caption')) {
                    ...
                    //Click function
                    $item.find('.sby_expand a').unbind('click').bind('click', function (e) {
                        e.preventDefault();
